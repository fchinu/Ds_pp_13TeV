...
for iPt, (ptMin, ptMax) in enumerate(zip(ptMins, ptMaxs)):
    ...
    dfBkgPt = dfBkg_tot.query(f'{ptMin} < fPt  and fPt < {ptMax}').sample(
    ...
    if dfSecPeakPrompt and dfSecPeakFD:
        hMassSecPeak = TH1F(f'hMassSignal_pT{ptMin}-{ptMax}', ';#it{M} (GeV/#it{c});Counts', 400,
                            min(dfSecPeakPrompt['fM']), max(dfSecPeakPrompt['fM']))
        for mass in np.concatenate((dfSecPeakPrompt['fM'].to_numpy(), dfSecPeakFD['fM'].to_numpy())):
    ...
    for iParCut, (ParCutMin, ParCutMax) in enumerate(zip(ParCutMins, ParCutMaxs)):
        ...
        if 1 <= len(varNames) <= 2:
            ...
            cEstimVsCut[counter].Print(args.outFileName.replace(".root",f'scan_pT{ptMin}_{ptMax}.pdf'))
        counter += 1
outFile.cd()
tSignif.Write()
outFile.Close()
